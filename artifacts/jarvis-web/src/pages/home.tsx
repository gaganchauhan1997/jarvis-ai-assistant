...
export default function Home() {
  ...
  const handleSpeechEnd = () => {
    if (transcript.trim()) {
      processTranscription(transcript.trim());
    }
  };
  ...
  const { isRecording, transcript, startRecording, stopRecording } = useSpeechRecognition({
    ...
    onError: (err) => {
      toast({ title: "Speech Recognition Error", description: err.message || "An error occurred", variant: "destructive" });
    }
  });

  const processTranscription = async (rawText: string) => {
    try {
      const result = await processDictation.mutateAsync({
        data: { rawText, mode }
      });
      ...
      // Auto save
      await saveDictation.mutateAsync({
        data: { rawText, processedText: result.processedText, mode: result.mode }
      });
            ...
            <CardContent className="p-8 flex flex-col items-center justify-center space-y-8">
              ...
              <div className="relative">
                ...
                <Button
                  ...
                  className={`w-32 h-32 rounded-full flex flex-col items-center justify-center space-y-2 transition-all duration-300 ${
                    isRecording ? 'animate-[pulse-ring_2s_infinite]' : 'hover:scale-105'
                  }`}
                ...
                {history.map((entry, i) => (
                  ...
                  >
                    <CardContent className="p-4 relative">
                      ...
                      <p className="text-sm line-clamp-3 mb-2">{entry.processedText}</p>
                      
                      <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex space-x-1 bg-background/80 backdrop-blur rounded p-1">
                        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => copyToClipboard(entry.processedText)}>
                          <Copy className="w-3 h-3" />
                        </Button>
                        <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive hover:text-destructive" onClick={() => handleDelete(entry.id)}>
